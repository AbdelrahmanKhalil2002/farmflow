...
class _AddAnimalScreenState extends ConsumerState<AddAnimalScreen> {
  ...
  Future<void> _pickExpectedBirthDate() async {
    ...
    if (picked != null) setState(() => _expectedBirthDate = picked);
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final prFields = <String, dynamic>{
        if (_gender == 'female' && _pregnancyStatus != 'none')
          'pregnancyStatus': _pregnancyStatus,
        if (_gender == 'female' && _pregnancyStatus == 'pregnant' &&
            _expectedBirthDate != null)
          'expectedBirthDate': _expectedBirthDate!.toIso8601String(),
      };
      if (_images.isEmpty) {
        await dio.post(ApiEndpoints.animals, data: {
          'type':      _type,
          'gender':    _gender,
          'tagId':     _tagCtrl.text.trim(),
          'birthDate': _birthDate!.toIso8601String(),
          if (_breedCtrl.text.trim().isNotEmpty)  'breed':  _breedCtrl.text.trim(),
          if (_weightCtrl.text.trim().isNotEmpty) 'currentWeight': double.tryParse(_weightCtrl.text),
        ...
        final formData = FormData.fromMap({
          ...
          'birthDate': _birthDate!.toIso8601String(),
          if (_breedCtrl.text.trim().isNotEmpty)  'breed':  _breedCtrl.text.trim(),
          if (_weightCtrl.text.trim().isNotEmpty) 'currentWeight': _weightCtrl.text.trim(),
          if (_colorCtrl.text.trim().isNotEmpty)  'color':  _colorCtrl.text.trim(),
          if (_notesCtrl.text.trim().isNotEmpty)  'notes':  _notesCtrl.text.trim(),
          ...prFields,
        });
        ...
        await dio.post(ApiEndpoints.animals, data: formData);
      }
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: _birthDate != null
                  ...
                  children: [
                    Icon(Icons.calendar_today_outlined,
                        ...
                        color: _birthDate != null
                            ? AppColors.green : AppColors.muted),
              ...
              Wrap(
                ...
                children: pregnancyStatuses.map((s) => GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: _pregnancyStatus == s.$1
                          ? AppColors.green : AppColors.card,
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(
                        color: _pregnancyStatus == s.$1
                            ? AppColors.green : AppColors.border,
