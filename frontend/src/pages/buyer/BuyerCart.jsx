...
const LIVE_META = {
  cattle:  { emoji: '🐄', labelKey: 'herd.type.cattle'  },
  buffalo: { emoji: '🐃', labelKey: 'herd.type.buffalo' },
  sheep:   { emoji: '🐑', labelKey: 'herd.type.sheep'   },
  goat:    { emoji: '🐐', labelKey: 'herd.type.goat'    },
  camel:   { emoji: '🐪', labelKey: 'herd.type.camel'   },
  horse:   { emoji: '🐎', labelKey: 'herd.type.horse'   },
  poultry: { emoji: '🐔', labelKey: 'herd.type.poultry' },
  rabbit:  { emoji: '🐇', labelKey: 'herd.type.rabbit'  },
  other:   { emoji: '🐾', labelKey: 'herd.type.other'   },
};

// ─── Step indicator ─────────────────────────────────────────────────────────
const StepBar = ({ step, tFn }) => {
  const steps = [tFn('cart.step1'), tFn('cart.step2'), tFn('cart.step3')];
  return (
...
const BuyerCart = () => {
  ...
  const PAYMENT_OPTS = [
    { key: 'cod',      icon: '💵', label: t('cart.pay.cod'),      sub: t('cart.pay.codSub')      },
    { key: 'instapay', icon: '📱', label: t('cart.pay.instapay'), sub: t('cart.pay.instapaySub') },
  ];

  const handlePlaceOrder = async () => {
    if (payment === 'instapay' && !instaRef.trim()) {
      setErrMsg(t('cart.instaRefRequired'));
      return;
    }
    setPlacing(true);
    setErrMsg('');
    try {
      ...
      setErrMsg(err?.response?.data?.message || t('cart.placeErr'));
    } finally {
      setPlacing(false);
    }
  };
  ...
  return (
    <div style={{ margin: '-24px', background: C.bg, minHeight: '100vh', fontFamily: "system-ui, -apple-system, 'Segoe UI', sans-serif" }} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div style={{ background: `linear-gradient(135deg, #3A7D44 0%, #2D6235 100%)`, padding: '24px 32px' }}>
        ...
        >
          {checkoutStep > 1 && checkoutStep < 3 ? `← ${t('common.back')}` : `← ${t('cart.continueShopping')}`}
        </button>
        <h1 style={{ margin: 0, fontSize: '22px', fontWeight: '900', color: '#fff' }}>🛒 {t('cart.title')}</h1>
        <p style={{ margin: '4px 0 0', fontSize: '13px', color: 'rgba(255,255,255,0.7)' }}>
          {checkoutStep === 3 ? t('cart.ordered') : `${items.length} ${t('cart.items')}`}
        </p>
      </div>

      <div style={{ padding: '24px 32px 56px', maxWidth: '860px', margin: '0 auto' }}>

        {checkoutStep < 3 && items.length > 0 && <StepBar step={checkoutStep} tFn={t} />}

        {/* ── STEP 3: CONFIRMATION ─────────────────────── */}
        {checkoutStep === 3 && (
          <div style={{ background: C.white, borderRadius: '20px', padding: '40px 32px', textAlign: 'center', boxShadow: C.shadow }}>
            <div style={{ fontSize: '64px', marginBottom: '16px' }}>✅</div>
            <h2 style={{ margin: '0 0 8px', fontSize: '22px', fontWeight: '900', color: C.text }}>{t('cart.success')}</h2>
            <p style={{ margin: '0 0 24px', fontSize: '14px', color: C.muted }}>{t('cart.successHint')}</p>

            <div style={{ background: '#F9F5F0', borderRadius: '14px', padding: '18px 24px', marginBottom: '28px', textAlign: isRTL ? 'right' : 'left' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontSize: '14px', color: C.muted }}>{t('cart.itemCount')}</span>
                <span style={{ fontSize: '14px', fontWeight: '700', color: C.text }}>{orderIds.length}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontSize: '14px', color: C.muted }}>{t('cart.payMethod')}</span>
                <span style={{ fontSize: '14px', fontWeight: '700', color: C.text }}>
                  {payment === 'instapay' ? `📱 ${t('cart.pay.instapay')}` : `💵 ${t('cart.pay.cod')}`}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: `1px solid ${C.border}`, paddingTop: '10px', marginTop: '4px' }}>
                <span style={{ fontSize: '15px', fontWeight: '700', color: C.text }}>{t('common.total')}</span>
                <span style={{ fontSize: '18px', fontWeight: '900', color: C.green }}>{fmt(total)} {t('common.egp')}</span>
              </div>
            </div>
          ...
          <>
            ...
            {items.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 24px', background: C.white, borderRadius: '16px', boxShadow: C.shadow }}>
                ...
                <p style={{ color: C.text, fontSize: '17px', fontWeight: '700', margin: '0 0 6px' }}>{t('cart.empty')}</p>
                <p style={{ color: C.muted, fontSize: '14px', margin: '0 0 20px' }}>{t('cart.emptyHint')}</p>
              ...
              <>
                    ...
                    return (
                      <div key={item._id} style={{ background: C.white, borderRadius: '14px', border: `1px solid ${C.border}`, boxShadow: C.shadow, display: 'flex', alignItems: 'center', gap: '14px', padding: '14px', overflow: 'hidden' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: '800', fontSize: '15px', color: C.text }}>{item.breed || t(meta.labelKey)}</div>
                          <div style={{ fontSize: '12px', color: C.muted, marginTop: '2px' }}>{meta.emoji} {t(meta.labelKey)}</div>
                          {item.age && (
                            <div style={{ fontSize: '11px', color: C.muted, marginTop: '3px' }}>📅 {item.age} {t('common.month')} · ⚖️ {item.weight} {t('common.kg')}</div>
                          )}
                        </div>
                        <div style={{ textAlign: 'center', flexShrink: 0 }}>
                          <div style={{ fontSize: '16px', fontWeight: '800', color: C.text }}>{fmt(item.price)}</div>
                          <div style={{ fontSize: '11px', color: C.muted }}>{t('common.egp')}</div>
                        </div>
                        <button type="button" onClick={() => removeItem(item._id)} aria-label={t('cart.removeItem')}
                ...
                <div style={{ background: C.white, borderRadius: '16px', border: `1px solid ${C.border}`, padding: '20px 24px', boxShadow: C.shadow }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    <span style={{ fontSize: '14px', color: C.muted, fontWeight: '600' }}>{t('common.total')} ({items.length} {t('cart.items')})</span>
                    <span style={{ fontSize: '22px', fontWeight: '900', color: C.text }}>{fmt(total)} <span style={{ fontSize: '13px', color: C.muted }}>{t('common.egp')}</span></span>
                  </div>
        ...
        {checkoutStep === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {/* Payment methods */}
            <div style={{ background: C.white, borderRadius: '16px', border: `1px solid ${C.border}`, padding: '20px 24px', boxShadow: C.shadow }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: '800', color: C.text }}>💳 {t('cart.payMethod')}</h3>
              ...
              {payment === 'instapay' && (
                <div style={{ marginTop: '16px', padding: '14px 16px', background: C.amberBg, borderRadius: '12px', border: `1px solid #FDE68A` }}>
                  <div style={{ fontSize: '13px', fontWeight: '700', color: C.amber, marginBottom: '8px' }}>
                    📋 {t('cart.instaTransferTo')}: <span style={{ fontSize: '15px', letterSpacing: '0.05em' }}>01234567890</span>
                  </div>
                  <input
                    type="text"
                    value={instaRef}
            ...
            {/* Delivery address */}
            <div style={{ background: C.white, borderRadius: '16px', border: `1px solid ${C.border}`, padding: '20px 24px', boxShadow: C.shadow }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: '800', color: C.text }}>📍 {t('cart.deliveryAddress')}</h3>
              <textarea
                value={address}
            ...
            <div style={{ background: C.white, borderRadius: '16px', border: `1px solid ${C.border}`, padding: '20px 24px', boxShadow: C.shadow }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <span style={{ fontSize: '14px', color: C.muted, fontWeight: '600' }}>{t('cart.orderTotal')} ({items.length} {t('cart.items')})</span>
                <span style={{ fontSize: '22px', fontWeight: '900', color: C.text }}>{fmt(total)} <span style={{ fontSize: '13px', color: C.muted }}>{t('common.egp')}</span></span>
              </div>
              <button type="button" onClick={handlePlaceOrder} disabled={placing}
                ...
                {placing ? t('cart.placing') : `✅ ${t('cart.confirmOrder')}`}
              </button>
            </div>
          </div>
        )}
